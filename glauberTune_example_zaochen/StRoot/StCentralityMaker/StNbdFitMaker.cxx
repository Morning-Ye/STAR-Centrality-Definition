	...
	for(Int_t ix=0; ix<nppbin; ix++){
		...
		for(Int_t iy=0; iy<kbin; iy++){
			...
			for(Int_t iz=0; iz<xbin; iz++){
				...
				LOG_INFO << Form("StNbdFitMaker::Scan  Fitting for (npp, k, x, chi2/ndf) = (%1.3f, %1.3f, %1.3f, %1.3f/%3d=%1.3f) ...", 
						npp, k, x, mChi2Graph->GetY()[0], (Int_t)mChi2Graph->GetY()[1], mChi2Graph->GetY()[2])
					<< endm;
			}// x loop
		}// k loop
	}// npp loop
